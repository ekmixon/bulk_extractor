...
def get_lines_array(f):
    ...
    line_number = 0
    line_numbers = []
    pattern = args.pattern.encode('utf-8') if args.pattern else None
    xpattern = args.xpattern.encode('utf-8') if args.xpattern else None
    for line in f:
        line_number += 1
        if pattern and pattern not in line.split(b"\t")[0]:
            continue
...
def sample(outdir,fn):
    ...
    print(f"{fn} has {len(line_numbers)} lines")
    ...
    with open(os.path.join(outdir,fn),"w") as out:
        ...
        out.write(f"# Sampled {count} out of {len(line_numbers)}\n")
...
def calc_stats(fn):
    ...
    for line in open(fn,"r"):
        if line[0]=='#':
            if m := re.search("# Sampled (\d+) out of (\d+)", line):
                sampled = int(m[1])
                total = int(m[2])
        elif line[0] in '-_xzn' :
            wrong += 1
            ...
            right += 1
        elif not args.quiet: print("No classification:",line,end='')
